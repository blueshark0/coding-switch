...
type Queries interface {
	ListRequestLogs(platform string, provider string, limit int) ([]observabilitydomain.RequestLog, error)
	ListProviders(platform string) ([]string, error)
	StatsSince(platform string, provider string) (observabilitydomain.LogStats, error)
...
func (s *Service) ListRequestLogs(platform string, provider string, limit int) ([]observabilitydomain.RequestLog, error) {
	return s.queries.ListRequestLogs(platform, provider, limit)
}
...
func (s *Service) StatsSince(platform string, provider string) (observabilitydomain.LogStats, error) {
	return s.queries.StatsSince(platform, provider)
}
